...
public abstract class BaseQueryFilter
{
    ...
    protected static void ScaleAndAddPrepRecipes(QueryResults recipe, HashSet<QueryResults> finalResults, HashSet<QueryResults> prepRecipes)
    {
        ...
        {
            double scale; // NOTE: PrepRecipe instances are shared by reference so be careful setting SetScale.
            ...
            if (someRecipeIngredientsGrams > 0 && prepRecipe.Key.Recipe.Measure == Measure.None)
            {
                ...
                var prepRecipeGrams = prepRecipe.Key.RecipeIngredients.Where(ri => ri.Type == RecipeIngredientType.Ingredient).Sum(ri => ri.GetGramsUsed) * prepRecipe.Key.Recipe.Servings;
                var noneScale = noneRecipeIngredientsGrams / prepRecipe.Key.Recipe.Measure.ToGramsOrMilliliters(prepRecipe.Key.Recipe.Servings);
                scale = noneScale + (someRecipeIngredientsGrams / prepRecipeGrams);
            }
            else
            {
                // Normal scaling, divide the sum of the recipe ingredient's quantities by the serving size scaled prerequisite quantity.
                scale = (noneRecipeIngredientsGrams + someRecipeIngredientsGrams) / prepRecipe.Key.Recipe.Measure.ToGramsOrMilliliters(prepRecipe.Key.Recipe.Servings);
            }
            ...
            {
                scalePrepRecipe.SetScale += scale;
            }
            ...
            {
                existingPrepRecipe.SetScale += scale;
            }
            else
            {
                prepRecipe.Key.SetScale = scale;
