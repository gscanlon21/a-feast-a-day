...
public class CreateFeasts : IJob, IScheduled
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        {
            _logger.Log(LogLevel.Information, "Ending job {p0}", nameof(CreateFeasts));
        }
    }
    ...
    internal async IAsyncEnumerable<(User User, string Token)> GetUsers()
    {
        ...
        foreach (var user in await _coreContext.Users.AsNoTracking()
            ...
            .Where(u => u.SendDay == currentDay || (u.Features.HasFlag(Features.Debug) && DebugConsts.DebugDays.Contains(u.SendDay)))
